...
async function checkAuth() {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('userAvatar').textContent = currentUser.full_name ? currentUser.full_name.charAt(0).toUpperCase() : currentUser.username.charAt(0).toUpperCase();
...
function displayResult(result) {
    ...
    resultCard.className = `result-card ${isPneumonia ? 'pneumonia' : 'normal'}`;
    resultIcon.innerHTML = `<i class="fas ${isPneumonia ? 'fa-exclamation-triangle' : 'fa-check-circle'}"></i>`;
    ...
    try {
        ...
        tbody.innerHTML = predictions.map(p => `
            <tr>
                ...
                <td><span class="badge ${p.result === 'Pneumonia' ? 'badge-danger' : 'badge-success'}">${p.result}</span></td>
                <td>${p.confidence}%</td>
                <td>
                    <button class="action-btn" onclick="viewPrediction(${p.id})"><i class="fas fa-eye"></i></button>
                    <button class="action-btn" onclick="downloadPredictionReport(${p.id})"><i class="fas fa-download"></i></button>
                </td>
            </tr>
        `).join('');
    } catch (error) {
...
async function viewPrediction(predictionId) {
    const response = await fetch(`${API_URL}/api/predictions/${predictionId}`, { credentials: 'include' });
    const pred = await response.json();
    alert(`Patient: ${pred.patient_name}\nResult: ${pred.result}\nConfidence: ${pred.confidence}%\nDate: ${new Date(pred.predicted_at).toLocaleString()}\nNotes: ${pred.notes || 'None'}`);
}

async function downloadPredictionReport(predictionId) {
